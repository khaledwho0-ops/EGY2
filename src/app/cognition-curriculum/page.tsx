...
const PHASE_ACCENT: Record<number, string> = { 1: 'var(--accent-indigo)', 2: 'var(--accent-blue)', 3: 'var(--accent-purple)', 4: 'var(--accent-emerald)' };
...
export default function CognitionCurriculumPage() {
  const { isRTL, setLanguage } = useRTL();
  ...
  return (
    <div dir={ar ? 'rtl' : 'ltr'} style={{ minHeight: '100vh', background: 'var(--bg-page)', color: 'var(--text-primary)', fontFamily: ar ? 'var(--font-heading-ar, Tahoma, sans-serif)' : "var(--font-body,'Inter',system-ui,sans-serif)" }}>
      <div style={{ maxWidth: 980, marginInline: 'auto', padding: '40px 22px 96px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
          <div>
            <div style={{ fontSize: 11, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--text-muted)', fontWeight: 700 }}>{t('The Cognition Build', 'بناء الإدراك')}</div>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: 15, maxWidth: 620, lineHeight: 1.6 }}>
          ...
          <button onClick={() => setLanguage(ar ? 'en' : 'ar')} style={{ flexShrink: 0, border: '1px solid var(--border-secondary)', background: 'var(--bg-elevated)', color: 'var(--text-primary)', borderRadius: 10, padding: '8px 14px', cursor: 'pointer', fontSize: 14, fontWeight: 600 }}>{ar ? 'EN' : 'ع'}</button>
        </div>

        <div style={{ display: 'flex', gap: 18, marginTop: 22, padding: '14px 18px', background: 'var(--bg-card)', border: '1px solid var(--border-primary)', borderRadius: 14, flexWrap: 'wrap' }}>
          ...
          <Stat label={t('One-Law sourced', 'موثّق بالقانون الواحد')} value="100%" accent="var(--accent-emerald)" />
          {profile && profile.days.length > 0 && (
            <>
              <Stat label={t('Your days', 'أيامك')} value={String(profile.days.length)} accent="var(--accent-indigo)" />
        ...
        {active ? (
          ...
          <div style={{ marginTop: 30, display: 'flex', flexDirection: 'column', gap: 30 }}>
            {phases.map((p) => (
              <section key={p.phase}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                  <span style={{ width: 10, height: 10, borderRadius: 999, background: PHASE_ACCENT[p.phase] || 'var(--accent-indigo)' }} />
                  <h2 style={{ fontSize: 18, fontWeight: 700 }}>{ar ? p.name.ar : p.name.en}</h2>
                  <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>· {p.days.length} {t('days', 'يوم')}</span>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 12 }}>
                  {p.days.map((d) => (
                    <button key={d.day} onClick={() => setActive(d)} style={{ textAlign: ar ? 'right' : 'left', background: 'var(--bg-card)', border: '1px solid var(--border-primary)', borderRadius: 14, padding: '14px 16px', cursor: 'pointer', color: 'var(--text-primary)' }}
                      onMouseEnter={(e) => (e.currentTarget.style.borderColor = PHASE_ACCENT[p.phase] || 'var(--accent-indigo)')}
            ...
            {!phases.length && <p style={{ color: 'var(--text-muted)' }}>{t('Curriculum is being authored — check back shortly.', 'المنهج بيتكتب دلوقتي — ارجع بعد شوية.')}</p>}
          </div>
        )}
      </div>
    </div>
...
function Stat({ label, value, accent }: { label: string; value: string; accent?: string }) {
  return (<div><div style={{ fontSize: 22, fontWeight: 700, color: accent || 'var(--text-primary)' }}>{value}</div><div style={{ fontSize: 11.5, color: 'var(--text-muted)' }}>{label}</div></div>);
}
...
function Player({ day, lang, onBack }: { day: DayMeta; lang: Lang; onBack: () => void }) {
  ...
  if (err) return <Shell onBack={onBack} t={t}><p style={{ color: 'var(--accent-red)' }}>{t('Error:', 'خطأ:')} {err}</p></Shell>;
  if (!data || !ex) return <Shell onBack={onBack} t={t}><p style={{ color: 'var(--text-muted)' }}>{t('Loading…', 'بيحمّل…')}</p></Shell>;
  ...
  if (finished) {
    ...
    return (
      <Shell onBack={onBack} t={t}>
        ...
        <p style={{ color: 'var(--text-secondary)', fontSize: 14, marginBottom: 18 }}>{ar ? (data.titleAr || data.title) : data.title}</p>
        <div style={{ display: 'flex', gap: 22, flexWrap: 'wrap', marginBottom: 18 }}>
          <Stat label={t('Score', 'النتيجة')} value={`${score}/${results.length}`} />
          <Stat label={t('Accuracy', 'الدقّة')} value={`${acc}%`} accent="var(--accent-emerald)" />
          <Stat label={t('Confidence calibration', 'معايرة الثقة')} value={results.some((r) => r.conf != null) ? (ar ? cal.labelAr : cal.label) : '—'} accent={cal.direction === 'over' ? 'var(--accent-red)' : cal.direction === 'under' ? 'var(--accent-amber)' : 'var(--accent-emerald)'} />
        </div>
        <p style={{ fontSize: 13.5, color: 'var(--text-secondary)', lineHeight: 1.6, background: 'var(--bg-card)', border: '1px solid var(--border-primary)', borderRadius: 12, padding: '12px 14px', margin: 0 }}>
          {cal.direction === 'over'
  ...
  return (
    <Shell onBack={onBack} t={t}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
        ...
        <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>{idx + 1}/{exercises.length} · {t('score', 'نتيجة')} {score}</span>
      </div>
      <div style={{ height: 4, background: 'var(--border-primary)', borderRadius: 9, overflow: 'hidden', marginBottom: 14 }}>
        <div style={{ width: `${((idx + (answered ? 1 : 0)) / exercises.length) * 100}%`, height: '100%', background: 'var(--accent-indigo)', transition: 'width .25s' }} />
      </div>
      <MechChip mech={mech} ar={ar} />

      {/* PROMPT */}
      {isTF && ex.myth_en && <div style={{ fontSize: 12, color: 'var(--accent-amber)', margin: '8px 0' }}>{t('Myth to judge:', 'خرافة للحكم:')} “{ar ? ex.myth_ar : ex.myth_en}”</div>}
      ...
      {isDiff && ex.question && <p style={{ fontSize: 14, color: 'var(--text-secondary)', marginBottom: 12 }}>{ar ? ex.questionAr : ex.question}</p>}

      {/* CALIBRATION SLIDER (before answering) */}
      {calibrate && picked === null && (
        <div style={{ marginBottom: 14, padding: '12px 14px', background: 'var(--bg-card)', border: '1px solid var(--border-primary)', borderRadius: 12 }}>
          <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 8 }}>{t('Before you answer — how sure are you?', 'قبل ما تجاوب — إنت متأكد قد إيه؟')} <strong style={{ color: 'var(--text-primary)' }}>{confidence}%</strong></div>
      ...
      {gradeable && optionRows.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 9, opacity: calibrate && !confLocked && picked === null ? 0.5 : 1, pointerEvents: calibrate && !confLocked && picked === null ? 'none' : 'auto' }}>
          {optionRows.map((o, i) => {
            ...
            const bg = picked === null ? 'var(--bg-elevated)' : o.ok ? 'rgba(52,211,153,.14)' : chosen ? 'rgba(248,113,113,.14)' : 'var(--bg-elevated)';
            const bd = picked === null ? 'var(--border-secondary)' : o.ok ? 'var(--accent-emerald)' : chosen ? 'var(--accent-red)' : 'var(--border-secondary)';
            return <button key={i} disabled={picked !== null} onClick={() => grade(o.val)} style={{ textAlign: ar ? 'right' : 'left', background: bg, border: `1px solid ${bd}`, color: 'var(--text-primary)', borderRadius: 11, padding: '12px 15px', cursor: picked !== null ? 'default' : 'pointer', fontSize: 14.5 }}>{o.label}{reveal && o.ok ? '  ✓' : reveal && chosen ? '  ✕' : ''}</button>;
          })}
        </div>
      )}
      ...
      {mech === 'decompose' && Array.isArray(ex.parts) && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {ex.parts.map((p, i) => {
            ...
            return (
              <div key={i} style={{ background: 'var(--bg-elevated)', border: `1px solid ${submitted ? (right ? 'var(--accent-emerald)' : 'var(--accent-red)') : 'var(--border-secondary)'}`, borderRadius: 11, padding: '11px 13px' }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {ROLES.map((r) => (
                    <button key={r} disabled={submitted} onClick={() => setRoles((m) => ({ ...m, [i]: r }))}
                      style={{ fontSize: 12, borderRadius: 8, padding: '5px 10px', cursor: submitted ? 'default' : 'pointer', border: `1px solid ${got === r ? 'var(--accent-indigo)' : 'var(--border-secondary)'}`, background: got === r ? 'rgba(139,92,246,.18)' : 'var(--bg-sidebar)', color: 'var(--text-primary)' }}>
                      {ar ? ROLE_AR[r] : r}
                    </button>
                  ))}
                </div>
                {submitted && !right && <div style={{ fontSize: 12, color: 'var(--accent-emerald)', marginTop: 6 }}>{t('Correct role:', 'الدور الصح:')} {ar ? ROLE_AR[p.role || ''] : p.role}</div>}
              </div>
            );
          })}
      ...
      {(mech === 'self_explain' || mech === 'consider_opposite' || mech === 'inoculate') && (gradeable ? picked !== null : true) && !revealed && (
        <div style={{ marginTop: 12 }}>
          ...
          <textarea value={text} onChange={(e) => setText(e.target.value)} rows={3} placeholder={t('Type your thinking…', 'اكتب تفكيرك…')} dir={ar ? 'rtl' : 'ltr'} style={{ width: '100%', background: 'var(--bg-sidebar)', border: '1px solid var(--border-secondary)', borderRadius: 10, color: 'var(--text-primary)', padding: '10px 12px', fontSize: 14, resize: 'vertical' }} />
      ...
      {/* FEEDBACK PANEL */}
      {answered && (
        <div style={{ marginTop: 16, padding: '14px 16px', background: 'var(--bg-card)', border: '1px solid var(--border-primary)', borderRadius: 12 }}>
          {calMsg && <div style={{ fontSize: 13, color: correct ? 'var(--accent-emerald)' : 'var(--accent-amber)', marginBottom: 8, fontWeight: 600 }}>{calMsg}</div>}
          {ex.cognitivebiasAtPlay && <div style={{ display: 'inline-block', fontSize: 11, color: 'var(--accent-purple)', border: '1px solid var(--border-secondary)', borderRadius: 999, padding: '2px 10px', marginBottom: 8 }}>🧠 {ex.cognitivebiasAtPlay}</div>}
          {explanation && <p style={{ fontSize: 14.5, lineHeight: 1.65, color: 'var(--text-secondary)' }}>{explanation}</p>}
          {(mech === 'consider_opposite') && (ar ? ex.modelPointsAr : ex.modelPoints)?.length ? (
            <><div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 8 }}>{t('What could disprove / re-explain it:', 'اللي ممكن يكذّبه / يفسّره تاني:')}</div>
            <ul style={{ margin: '4px 0 0', paddingInlineStart: 18, color: 'var(--text-secondary)', fontSize: 13.5 }}>{(ar ? ex.modelPointsAr! : ex.modelPoints!).map((k, i) => <li key={i}>{k}</li>)}</ul></>
          ) : null}
          {(mech === 'inoculate') && (ar ? ex.tellsAr : ex.tells)?.length ? (
            <><div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 8 }}>{t('The tells that expose this technique:', 'العلامات اللي بتكشف الحركة دي:')}</div>
            <ul style={{ margin: '4px 0 0', paddingInlineStart: 18, color: 'var(--text-secondary)', fontSize: 13.5 }}>{(ar ? ex.tellsAr! : ex.tells!).map((k, i) => <li key={i}>{k}</li>)}</ul></>
          ) : null}
          {isDiff && ex.key_differentiators?.length ? <ul style={{ margin: '8px 0 0', paddingInlineStart: 18, color: 'var(--text-secondary)', fontSize: 13 }}>{ex.key_differentiators.map((k, i) => <li key={i}>{k}</li>)}</ul> : null}
          {ex.sources?.length ? (
            <div style={{ marginTop: 10, fontSize: 12, color: 'var(--text-muted)' }}>{t('Sources:', 'المصادر:')}{' '}
              {ex.sources.map((s, i) => <span key={i}>{s.startsWith('http') ? <a href={s} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--accent-blue)' }}>[{i + 1}]</a> : <span style={{ color: 'var(--text-secondary)' }}>[{i + 1} {s}]</span>}{' '}</span>)}
            </div>
          ) : null}
...
const btn: React.CSSProperties = { background: 'var(--accent-indigo)', color: '#fff', border: 'none', borderRadius: 10, padding: '9px 18px', cursor: 'pointer', fontSize: 14, fontWeight: 600 };

function Shell({ children, onBack, t }: { children: React.ReactNode; onBack: () => void; t: (en: string, ar: string) => string }) {
  return (
    <div style={{ marginTop: 26 }}>
      <button onClick={onBack} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: 13, marginBottom: 14 }}>← {t('Back to curriculum', 'رجوع للمنهج')}</button>
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-primary)', borderRadius: 18, padding: '22px 24px' }}>{children}</div>
    </div>
  );
}
